...
class AvgVopen(Feature):
    def calc(self):
        ...
        for i in range(len(self.maxPointX)):
            result.append(
                (self.maxPointY[i] - self.minPointY[i]) / (self.maxPointX[i] - self.minPointX[i])
            )
        if len(result) != 0:
...
class AvgVclose(Feature):
    def calc(self):
        ...
        for i in range(len(self.maxPointX)):
            result.append(
                (self.maxPointY[i] - self.minPointY[i + 1])
                / (self.minPointX[i + 1] - self.maxPointX[i])
            )
        if len(result) != 0:
            return sum(result) / len(result)
        else:
...
class DecA(Feature):
    def calc(self):
        ...
        maxPointX, maxPointY, minPointX, minPointY = self.loadfileInterval_hand(
            self.datapoint, start1, stop1
        )
        amplitude1 = AvgA(
            maxPointX, maxPointY, minPointX, minPointY, self.norm_coef, self.datapoint, self.config
        ).calc()

        start4 = round(3 * (max(self.minPointX) - start1) / 4 + start1)
        stop4 = round(max(self.minPointX) + 1)
        maxPointX, maxPointY, minPointX, minPointY = self.loadfileInterval_hand(
            self.datapoint, start4, stop4
        )
        amplitude4 = AvgA(
            maxPointX, maxPointY, minPointX, minPointY, self.norm_coef, self.datapoint, self.config
        ).calc()
...
class DecFA(Feature):
    def amplitudeDec(self, maxPointX, maxPointY, minPointX, minPointY):
        ...
        for i in range(len(minPointX) - 1):
            result.append(
                round(np.mean([maxPointY[i] - minPointY[i], maxPointY[i] - minPointY[i + 1]]), 2)
            )
        if result != 0:
            return result
        else:
        ...
        for i in range(len(minPointX) - 1):
            result.append(
                round(
                    np.mean(
                        [
                            (maxPointY[i] - minPointY[i + 1]) / (minPointX[i + 1] - maxPointX[i]),
                            (maxPointY[i] - minPointY[i]) / (maxPointX[i] - minPointX[i]),
                        ]
                    ),
                    2,
                )
