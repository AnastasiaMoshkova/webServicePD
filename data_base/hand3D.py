...
class HandData(HandBase):
    ...
    def signal_FT(self, data, hand):
        ...
        for i in range(len(data)):
            if hand in data[i].keys():
                sum_sqr = (
                    (
                        float(data[i][hand]["FORE_TIP"]["X1"])
                        - float(data[i][hand]["THUMB_TIP"]["X1"])
                    )
                    ** 2
                    + (
                        float(data[i][hand]["FORE_TIP"]["Y1"])
                        - float(data[i][hand]["THUMB_TIP"]["Y1"])
                    )
                    ** 2
                    + (
                        float(data[i][hand]["FORE_TIP"]["Z1"])
                        - float(data[i][hand]["THUMB_TIP"]["Z1"])
                    )
                    ** 2
                )
    ...
    def signal_OC(self, data, hand):
        ...
        for i in range(len(data)):
            if hand in data[i].keys():
                sum_sqr = (
                    (float(data[i][hand]["MIDDLE_TIP"]["X1"]) - float(data[i][hand]["CENTRE"]["X"]))
                    ** 2
                    + (
                        float(data[i][hand]["MIDDLE_TIP"]["Y1"])
                        - float(data[i][hand]["CENTRE"]["Y"])
                    )
                    ** 2
                    + (
                        float(data[i][hand]["MIDDLE_TIP"]["Z1"])
                        - float(data[i][hand]["CENTRE"]["Z"])
                    )
                    ** 2
                )
                distance = math.sqrt(sum_sqr)
